...
// Uniforms (matching mdx-m3-viewer exactly)
uniform mat4 u_VP;
...
// Attributes (matching mdx-m3-viewer exactly)
...
void main() {
  ...
  // Check if ANY texture layer is valid (mdx-m3-viewer logic)
  if (textures[0] > 0.0 || textures[1] > 0.0 || textures[2] > 0.0 || textures[3] > 0.0) {
    v_tilesets = textures;

    // Calculate UVs for each texture layer
    v_uv[0] = getUV(uv, u_extended[int(textures[0]) - 1] > 0.5, a_variations[0]);
    v_uv[1] = getUV(uv, u_extended[int(textures[1]) - 1] > 0.5, a_variations[1]);
    v_uv[2] = getUV(uv, u_extended[int(textures[2]) - 1] > 0.5, a_variations[2]);
    v_uv[3] = getUV(uv, u_extended[int(textures[3]) - 1] > 0.5, a_variations[3]);

    // Calculate position from instance ID
    vec2 corner = vec2(mod(a_InstanceID, u_size.x), floor(a_InstanceID / u_size.x));
    vec2 base = corner + vec2(position.x, position.z);
    ...
    // Final position calculation (exactly matching mdx-m3-viewer)
    gl_Position = u_VP * vec4(base * 128.0 + u_offset, height * 128.0, 1.0);
  } else {
    // Tile has no textures - don't render it
    v_tilesets = vec4(0.0);
    v_uv[0] = vec2(0.0);
    v_uv[1] = vec2(0.0);
    v_uv[2] = vec2(0.0);
