...
vec2 getUV(vec2 pos, bool extended, float variation) {
...
void main() {
  ...
  if (textures[0] > 0.0 || textures[1] > 0.0 || textures[2] > 0.0 || textures[3] > 0.0) {
    ...
    v_uv[0] = getUV(position, u_extended[int(textures[0]) - 1], a_variations[0]);
    v_uv[1] = getUV(position, u_extended[int(textures[1]) - 1], a_variations[1]);
    v_uv[2] = getUV(position, u_extended[int(textures[2]) - 1], a_variations[2]);
    v_uv[3] = getUV(position, u_extended[int(textures[3]) - 1], a_variations[3]);
