...
vec4 sample(float tileset, vec2 uv) {
  // 1.0 - 1.0 == 0.0 is not always true (floating point precision issue)
  ...
  return vec4(1.0, 0.0, 1.0, 1.0); // Fallback magenta
}
...
void main() {
  // ALWAYS sample the first texture layer
  ...
  // Lighting is commented out in mdx-m3-viewer
