...
export class LegalCompliancePipeline {
    ...
    try {
      ...
      if (!hashResult.valid) {
        ...
        if (this.config.autoReplace) {
          return this.findReplacement(metadata, warnings);
        } else {
      ...
      if (
        ...
        const similarityResult = this.checkVisualSimilarity(asset, metadata);
  ...
  private checkVisualSimilarity(
    ...
    try {
      ...
      const result = this.visualSimilarity.findSimilarInDatabase(
        asset,
        database,
   ...
   */
  private findReplacement(metadata: AssetMetadata, warnings: string[]): ValidatedAsset {
